      ...
      <Link to={fullRoute.aGlobalRoute.aUnprotectedRoute.aHomeRoute} >
        <div className="flex items-center space-x-2">
          <Avatar>
            ...
            <AvatarFallback>{getInitials(user ? `${user.eFirstname}` : "Kisna", user ? `${user.eLastname}` : "Kisna"  )}</AvatarFallback>
          </Avatar>
          <div className='hidden sm:block' >
            <p className="text-sm font-medium leading-none">{user ? `${user.eFirstname} ${user.eLastname}` : `Kisna`}</p>
            {/* <p className="text-sm text-muted-foreground">{user ? `${user.eEmail}` : `someone@kisna.com`}</p> */}
          </div>
        </div>
      </Link>
    </React.Fragment>
